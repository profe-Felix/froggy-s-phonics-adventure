...
export default function Desk() {
  ...
  const handleAddDesk = async () => {
    ...
    const newDesk = { class_name: selectedClass, group, x: 150 + offset, y: 150 + offset, rotation: 0, student_id: null };
    setSaving(true);
    try {
  ...
  const handleAddLandmark = async () => {
    const newLm = { class_name: selectedClass, group, x: 300, y: 300, width: 80, height: 60, label: '' };
    setSaving(true);
    try {
      const created = await base44.entities.DeskLandmark.create(newLm);
  ...
  const handlePaste = useCallback(async () => {
    ...
    try {
      const newDesks = clipboard.map((d) => ({
        class_name: selectedClass, group, x: d.x + 24, y: d.y + 24, rotation: d.rotation, student_id: null,
      }));
  ...
  const startLandmarkDrag = (e, landmark) => {
    e.stopPropagation();
    e.preventDefault();
    const dragState = { landmarkId: landmark.id, startMouseX: e.clientX, startMouseY: e.clientY, startX: landmark.x, startY: landmark.y, moved: false, currentX: landmark.x, currentY: landmark.y };
    const handleMove = (ev) => {
      const dx = ev.clientX - dragState.startMouseX;
      const dy = ev.clientY - dragState.startMouseY;
      if (Math.abs(dx) > 3 || Math.abs(dy) > 3) dragState.moved = true;
      ...
      dragState.currentX = newX; dragState.currentY = newY;
    ...
    const handleUp = () => {
      ...
      if (dragState.moved) {
        ...
        base44.entities.DeskLandmark.update(landmark.id, { x: dragState.currentX, y: dragState.currentY })
          .then(() => setSaving(false)).catch(() => { loadRef.current(); setSaving(false); });
      }
    };
    document.addEventListener('pointermove', handleMove);
    document.addEventListener('pointerup', handleUp);
  ...
  const handleCanvasPointerDown = (e) => {
    ...
    if (drawMode && e.target === e.currentTarget) {
      ...
      const handleMove = (ev) => {
        ...
        drawingRef.current.currentX = cx;
        drawingRef.current.currentY = cy;
        setDrawing({
          x: Math.min(drawingRef.current.startX, cx),
          y: Math.min(drawingRef.current.startY, cy),
          w: Math.abs(cx - drawingRef.current.startX),
          h: Math.abs(cy - drawingRef.current.startY),
        });
      };
      const handleUp = async () => {
        ...
        setDrawing(null);
        if (d && Math.abs(d.currentX - d.startX) > 20 && Math.abs(d.currentY - d.startY) > 20) {
          setSaving(true);
          try {
            const created = await base44.entities.DeskLandmark.create({
              class_name: selectedClass, group,
              x: Math.round((d.startX + d.currentX) / 2 / GRID_SIZE) * GRID_SIZE,
              y: Math.round((d.startY + d.currentY) / 2 / GRID_SIZE) * GRID_SIZE,
              width: Math.round(Math.abs(d.currentX - d.startX) / GRID_SIZE) * GRID_SIZE,
              height: Math.round(Math.abs(d.currentY - d.startY) / GRID_SIZE) * GRID_SIZE,
              label: '',
            });
            setLandmarks((prev) => [...(prev || []), created]);
          } catch { loadRef.current(); }
  ...
  const hint = drawMode
    ? 'Draw a rectangle to add a landmark (teacher desk, door…). Double-click a landmark to label it.'
    : selectedBankStudent
      ? 'Tap a desk to seat the selected student.'
      : swapSourceId
        ? 'Tap another desk to swap — or tap the highlighted one to cancel.'
        : 'Drag desks to move · Ctrl+click to multi-select · Tap a student then a desk to seat · Tap a seated desk to swap.';
    ...
    <div className="min-h-screen bg-slate-100">
      <header className="bg-white border-b sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold leading-tight">Desk Layout</h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button size="sm" variant="outline" onClick={handleAddDesk}><Plus className="w-4 h-4 mr-1" /> Desk</Button>
            <Button size="sm" variant="outline" onClick={handleAddLandmark}>Landmark</Button>
            <Button size="sm" variant="outline" onClick={handleReset}><RefreshCw className="w-4 h-4 mr-1" /> Reset</Button>
            <Button size="sm" variant="outline" onClick={handleCopyLayoutToGroups}><Copy className="w-4 h-4 mr-1" /> Copy A→B,C</Button>
            <Button size="sm" variant={drawMode ? 'default' : 'outline'} onClick={toggleDrawMode}><Pencil className="w-4 h-4 mr-1" /> Draw</Button>
            <Button size="sm" variant="outline" onClick={handleCopy} disabled={selectedDeskIds.size === 0}><ClipboardPaste className="w-4 h-4 mr-1" /> Copy</Button>
            <Button size="sm" variant="outline" onClick={handlePaste} disabled={clipboard.length === 0}><ClipboardPaste className="w-4 h-4 mr-1" /> Paste</Button>
      ...
      <main className="max-w-7xl mx-auto p-4 flex gap-4">
        {selectedClass && (
          <div className="w-56 shrink-0">
            <div className="bg-white border rounded-lg p-3">
              ...
              {!selectedBankStudent && !swapSourceId && (
                <p className="text-xs text-muted-foreground mb-2">Tap a desk to seat.</p>
              )}
              {swapSourceId && (() => {
                ...
                return (
                  <div className="mb-2">
                    <p className="text-xs text-muted-foreground">Tap another desk to swap — or tap the highlighted one to cancel.</p>
                    {st && (
                      <button onClick={() => handleUnseat(sd)} className="mt-1.5 text-xs text-destructive hover:text-destructive/80 inline-flex items-center gap-1">
        ...
        <div className="flex-1 overflow-auto">
          <div onPointerDown={handleCanvasPointerDown} className="relative bg-white rounded-lg border-2 border-dashed border-slate-200" style={{ width: CANVAS_W, height: CANVAS_H, minWidth: CANVAS_W }}>
            {desks && desks.length === 0 && (!landmarks || landmarks.length === 0) && (
              <div className="absolute inset-0 flex items-center justify-center text-sm text-muted-foreground">Click "Desk" to start arranging.</div>
            )}
            {(landmarks || []).map((lm) => (
              <DeskLandmarkItem key={lm.id} landmark={lm} onPointerDown={!drawMode ? (e) => startLandmarkDrag(e, lm) : undefined} onDelete={() => handleDeleteLandmark(lm)} onEdit={() => handleEditLandmark(lm)} />
            ))}
            {(desks || []).map((desk) => (
              <DeskItem key={desk.id} desk={desk} student={desk.student_id ? studentMap[desk.student_id] : null}
                isBankSelected={!!selectedBankStudent} isSelected={selectedDeskIds.has(desk.id)} isSwapSource={swapSourceId === desk.id}
                interactive={!drawMode} onPointerDown={(e) => handleDeskPointerDown(e, desk)} onRotate={() => handleRotate(desk)} onDelete={() => handleDelete(desk)} />
            ))}
            {drawing && <div className="absolute border-2 border-dashed border-amber-500 bg-amber-100/40 pointer-events-none rounded-sm" style={{ left: drawing.x, top: drawing.y, width: drawing.w, height: drawing.h }} />}
          </div>
