...
// Canvas-based Elkonin counting. Eight fixed boxes sit in a row; one chip rests
// below each box. The student drags a chip STRAIGHT UP into the box above it to
// count one unit (word/phoneme). Chips the student doesn't move stay put — no
// sliding, no reordering. Each drag is recorded as a timestamped motion path so
// the teacher can replay the chip rising into its box at the exact pace.
const BOX_COUNT = 8;
const BOX_TOP = 0.12, BOX_BOTTOM = 0.46, BOX_CENTER_Y = (BOX_TOP + BOX_BOTTOM) / 2, HOME_Y = 0.80;
const DROP_THRESHOLD_Y = 0.60; // release above this y -> placed; below -> back home
...
export default function ElkoninCountActivity({ config, studentName }) {
  ...
  function draw() {
    ...
    for (let i = 0; i < BOX_COUNT; i++) {
      ...
      if (dragRef.current && dragRef.current.chip === i) {
        ...
        cx = colX(i) * w; cy = (p[i] ? BOX_CENTER_Y : HOME_Y) * h;
      }
      drawChip(ctx, cx, cy, chipR);
    }
  }
  ...
  function onPointerDown(e) {
    ...
    const i = Math.min(BOX_COUNT - 1, Math.max(0, Math.floor((px / w) * BOX_COUNT)));
    const p = placedRef.current;
    const curY = p[i] ? BOX_CENTER_Y : HOME_Y;
    const chipR = (w / BOX_COUNT) * 0.34;
    if (Math.abs(py - curY * h) < chipR * 1.6) {
      canvasRef.current.setPointerCapture(e.pointerId);
      const t = recT();
      dragRef.current = { chip: i, fromPlaced: p[i], x: colX(i), y: curY, path: [{ t, x: colX(i), y: curY }] };
    }
  }
  ...
  function onPointerUp() {
    ...
    if (!d.fromPlaced && d.y < DROP_THRESHOLD_Y) placedAfter = true;
    else if (d.fromPlaced && d.y > DROP_THRESHOLD_Y) placedAfter = false;
    const finalY = placedAfter ? BOX_CENTER_Y : HOME_Y;
  ...
  return (
    <div className="flex flex-col gap-4 p-4 max-w-3xl mx-auto">
      ...
      <div>
        ...
        <div className="w-full rounded-xl bg-white border-2 border-slate-200" style={{ height: 240 }}>
          <canvas
            ref={canvasRef}
