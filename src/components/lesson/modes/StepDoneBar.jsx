...
export default function StepDoneBar({ onDone, label = 'Done', disabled = false }) {
  return (
    <div className="shrink-0 flex justify-center py-2 bg-white border-t">
      <button
        onClick={onDone}
        disabled={disabled}
        className="px-6 py-2 bg-green-500 text-white font-black rounded-2xl shadow hover:bg-green-600 inline-flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
      >
