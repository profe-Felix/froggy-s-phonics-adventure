...
// Adaptive sizing: keep every tracing level large enough for muscle-memory practice
// on an iPad. Students can still level up, but later levels no longer shrink to
...
export default function LetterTracingMode({ studentData, onUpdateProgress, targets }) {
          ...
          for (const r of records) {
            ...
            try {
              ...
              if (Array.isArray(strokes) && strokes.length) {
                merged[r.letter] = {
                  strokes,
                  hint: r.hint || prev[r.letter]?.hint || ''
                };
              }
            } catch {
              /* ignore malformed */
  ...
  const renderWidthFor = (l) => {
    const targetWidth = SIZE_LEVELS[levelFor(l)].w;
    ...
    // Use almost all of the available iPad width.
    //
    // The original version only used 92% of the viewport and eventually
    // allowed letters to shrink all the way down to 280px.
    //
    // For muscle-memory practice we want the student's hand/arm making
    // substantially larger movements, so every level remains large.
  ...
  const handleComplete = (letter) => {
    ...
    if (onUpdateProgress) {
      onUpdateProgress('letter_tracing', {
        total_attempts: traceCountRef.current
      });
    }
    ...
    if (
      ...
      // Struggled → return to a larger tracing size.
      next = cur - 1;
    }
    ...
    // Auto-advance to the next letter on the page instead of bouncing
    // back to the grid each time.
    const idx = paged.indexOf(letter);

    const nextLetter =
      idx >= 0 && idx < paged.length - 1
        ? paged[idx + 1]
        : letter;
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center py-6 px-4 gap-4">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Tap a letter to practice writing it
          </p>
        </div>
        ...
        <p className="text-slate-400 text-xs">
          {completedLetters.size}/{LETTERS.length} letters practiced
        </p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-4 px-4 gap-3">
      ...
      <div className="flex items-center justify-between w-full max-w-md">
        ...
        <div className="text-xs font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-full px-2 py-1">
          {sizeLabel} · L{lvl + 1}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
          ...
          <div className="bg-white rounded-3xl shadow-2xl px-8 py-6 flex flex-col items-center gap-2">
            ...
            <div className="text-slate-500 text-sm">
              {celebrate.letter.toUpperCase()} → {celebrate.label}
            </div>
