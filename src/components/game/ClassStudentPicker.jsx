...
/**
 * Reusable class + student picker matching the LetterGame login style:
 * grade-grouped gradient class tiles, then student number tiles with
 * login photos when available.
 *
 * Props:
 *  - onSelect(className, number)  — called when a student tile is tapped
 *  - preselectedClass             — skip straight to the number grid
 *  - icon, title, titleFrom, titleTo — StudentLoginShell branding
 */
export default function ClassStudentPicker({
  onSelect,
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                    {groups[grade].map((cls, i) => {
                      ...
                      return (
                        ...
                        >
                          <span className="absolute top-2 left-3 text-lg sm:text-xl opacity-70 group-hover:opacity-100 transition">{icon}</span>
          ...
          {NUMBERS.map((num, i) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.92 }}
                onClick={() => onSelect(selectedClass, num)}
