...
export default function AutofitName({ first, last, maxFontSize = 20, minFontSize = 7, className = '' }) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center leading-tight w-full ${className}`}>
      <div ref={firstRef} className="font-bold text-black text-center whitespace-nowrap w-full overflow-hidden">{first}</div>
      {last && <div ref={lastRef} className="font-bold text-black text-center whitespace-nowrap w-full overflow-hidden">{last}</div>}
    </div>
  );
}