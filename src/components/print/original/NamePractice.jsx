...
import NamePracticeSheet from '@/components/NamePracticeSheet';
...
export default function NamePractice() {
  ...
  // Load global settings (shared across all teachers, no login required)
  useEffect(() => {
    (async () => {
      try {
  ...
  const teachers = students
    ? [...new Set(students.map((s) => s.teacher_name || s.homeroom).filter(Boolean))].sort((a, b) => a.localeCompare(b))
    : [];

  let visible = students ?? [];
  if (teacherParam) visible = visible.filter((s) => (s.teacher_name || '').toLowerCase() === teacherParam.toLowerCase());
  if (homeroomParam) visible = visible.filter((s) => (s.homeroom || '').toLowerCase() === homeroomParam.toLowerCase());
  if (classFilter) visible = visible.filter((s) => (s.teacher_name || s.homeroom || '').toLowerCase() === classFilter.toLowerCase());

  useEffect(() => {
    if (teacherParam && !classFilter) setClassFilter(teacherParam);
  }, [teacherParam, classFilter]);

  useEffect(() => {
    if (visible.length && !visible.find((s) => s.id === selectedId)) {
      setSelectedId(visible[0].id);
    }
  }, [students, teacherParam, homeroomParam, classFilter, selectedId]);
        ...
        <div className="max-w-5xl mx-auto px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <Button asChild variant="ghost" size="icon">
              <Link to={rosterTarget}><ArrowLeft className="w-4 h-4" /></Link>
            </Button>
            <div>
