...
import DeskItem, { DESK_W, DESK_H } from '@/components/DeskItem';
import DeskLandmarkItem from '@/components/DeskLandmarkItem';
import StudentBankCard from '@/components/StudentBankCard';
import { getHomeroomForClass } from '@/lib/classRotation';
import { Loader2, ArrowLeft, Plus, RefreshCw, Users, Pencil, Copy, ClipboardPaste, Check, Trash2 } from 'lucide-react';
...
export default function Desk() {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [allDesks, allLandmarks] = await Promise.all([
      base44.entities.DeskSeat.filter({ teacher_name: teacher, group }),
      base44.entities.DeskLandmark.filter({ teacher_name: teacher, group }),
    ]);
    setDesks(allDesks);
  ...
  const handleRotate = async (desk) => {
    ...
    setSaving(true);
    try {
      await base44.entities.DeskSeat.update(desk.id, { rotation: newRotation });
    } catch {
      loadRef.current();
    }
    setSaving(false);
  ...
  const handleDelete = async (desk) => {
    ...
    try {
      await base44.entities.DeskSeat.delete(desk.id);
    } catch {
      loadRef.current();
    }
  };
  ...
  const handleUnseat = async (desk) => {
    ...
    setSaving(true);
    try {
      await base44.entities.DeskSeat.update(desk.id, { student_id: null });
    } catch {
      loadRef.current();
    }
    setSaving(false);
  ...
  const handleCopyLayoutToGroups = async () => {
    if (!teacher) return;
    if (!window.confirm(`Copy ${teacher}'s Group A desk layout to Groups B and C? This replaces any existing desks in B and C.`)) return;
    setSaving(true);
    try {
      const groupADesks = await base44.entities.DeskSeat.filter({ teacher_name: teacher, group: 'A' });
      if (groupADesks.length === 0) {
        setSaving(false);
        return;
      }
      await base44.entities.DeskSeat.deleteMany({ teacher_name: teacher, group: 'B' });
      await base44.entities.DeskSeat.deleteMany({ teacher_name: teacher, group: 'C' });
      const newDesks = [];
      for (const g of ['B', 'C']) {
        for (const d of groupADesks) {
          newDesks.push({ teacher_name: teacher, group: g, x: d.x, y: d.y, rotation: d.rotation || 0, student_id: null });
        }
      }
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (isCtrl && e.key === 'c' && selectedDeskIds.size > 0) {
        e.preventDefault();
        handleCopy();
      }
      if (isCtrl && e.key === 'v' && clipboard.length > 0) {
        e.preventDefault();
        handlePaste();
      }
      if (isCtrl && e.key === 'a') {
        e.preventDefault();
        setSelectedDeskIds(new Set((desks || []).map((d) => d.id)));
      }
      if (e.key === 'Escape') {
        setSelectedDeskIds(new Set());
        setSwapSourceId(null);
  ...
  const handleDeleteLandmark = async (landmark) => {
    ...
    try {
      await base44.entities.DeskLandmark.delete(landmark.id);
    } catch {
      loadRef.current();
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        {!teacher ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground">Select a teacher to start arranging desks.</p>
          </div>
        ) : desks === null ? (
                  ...
                  return (
                    <div className="mb-2">
                      ...
                      {st && (
                        ...
                        >
                          <Trash2 className="w-3.5 h-3.5" /> Unseat {st.student_name}
                        </button>
                      )}
                    </div>
                  );
